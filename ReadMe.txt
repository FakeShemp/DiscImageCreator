...
*Recommend Drive
CD: PLEXTOR(not OEM Drive - PX-708A, PX-712A, PX-716A, PX-755A, PX-760A)
...
==============================Ripping information==============================
*Ripping Guide for CD/DVD
Run exe without args.
...
6. run below. (rip gdrom)
   DiscImageCreator.exe -rgd [DriveLetter] [DriveSpeed(0-72)] foo.bin

=========================Known issue (fix if possible)=========================
. drive not ready
 Sometimes it will just abort with a drive not ready error. I have to open
 isobuster and hit refresh a couple of times before it works again.
 Also, error is "Unable to Access Device" happens randomly.
 
. Get exclusive access to the drive when ripping (like many burning programs do
  when burning), so that another programs cannot disturb the drive and the re-reads.

. regarding USB/IEEE-1394 drives: it seems that these drives sometimes require
 more or less 8 KB or lower transfers so that they can work OK, specially when
 C2 error reporting is enabled. You could add an option to limit the transfers
 to a maximum of 3 sectors for compatibility reasons: (2352 bytes +294 bytes
 +96 bytes)*3 = 8226 bytes.

. C2 error fixing Algorithm (because exists bug)
=============================Known issue (not fix)=============================
. reading with/without subcode, differ offset. (firmware bug?)
 PX-W2410TA: +686(with), +98(without)
...
. FUA ripping for CD.
 This flag can enable at READXX (READ12, READ16..) command.
 But READ_CD command (include 0xd8 command) doesn't exist this flag.

. Ripping the protected disc.
 Especially, ProRing, SecuROM. But I don't know week sector of SecuROM in detail.

. a routine to verify CDmage if a given data track extracted contains
 corruption according to the ECC/EDC stored, and report that in the 
 "_errorlog.txt" file or similar.

. cue file for multisession disc
 SESSION directives
