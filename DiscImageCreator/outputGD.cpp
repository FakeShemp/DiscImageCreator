	...
	)
{
	...
	try {
...
#endif
		INT nMaxToc = 98 * 4;
		UINT nMaxTrackNum = byToc[nMaxToc+4*1+2];
		pToc = (PLONG)calloc(nMaxTrackNum, sizeof(UINT));
		if(!pToc) {
		...
		for(UINT i = 3; i <= nMaxTrackNum; i++) {
			...
			if(NULL == (pBuf = (PUCHAR)calloc(size, sizeof(UCHAR)))) {
